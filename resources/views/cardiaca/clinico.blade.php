  ...
  <body>
    ...
    <div class="patient-card">
        ...
        <table class="patient-table">
            ...
            <tr>
                <td><span class="patient-label">Profesión:</span> <span class="patient-value">{{ $paciente->profesion }}</span></td>
                <td><span class="patient-label">F. Nacimiento:</span> <span class="patient-value">{{ $paciente->fechaNacimiento }}</span></td>
                <td><span class="patient-label">Ingreso (1vez):</span> <span class="patient-value">{{ date('d/m/Y', strtotime($data->fecha_1vez)) }}</span></td>
                <td><span class="patient-label">Estratificación:</span> <span class="patient-value">{{ date('d/m/Y', strtotime($data->estratificacion)) }}</span></td>
                <td colspan="2"><span class="patient-label">Domicilio:</span> <span class="patient-value">{{ $paciente->domicilio }}</span> &nbsp; <span class="patient-label">Tel:</span> <span class="patient-value">{{ $paciente->telefono }}</span></td>
            </tr>
        </table>
    ...
    <main class="mt-0">
        ...
        <p  class="f-bold m-t-0 f-10 mb-0">Fecha: <span class="f-normal">{{ $data->mn_fecha===null?"no tiene":date('d/m/Y',strtotime($data->mn_fecha))}}</span> <span class="f-bold ml-5">  FE: <span class="f-normal">{{$data->fe_por_mn}}</span></span>
            ...
            <p  class="f-bold m-t-0 f-10 mb-0 bck-gray">Fecha: <span class="f-normal">{{ $data->catet_fecha===null?"no tiene":date('d/m/Y',strtotime($data->catet_fecha))}}</span> <span class="f-bold ml-5">  FE: <span class="f-normal">{{$data->catet_fe===null?0:$data->catet_fe}}</span></span>
                ...
                <p class="f-bold mt-2 mb-0 f-15">Elaboró: <span class="f-normal">{{ $autor->nombre_completo }}</span></p>
                @if($autor->cedula)
                <p class="f-10 mb-0" style="color: #64748b;">Cédula Profesional: {{ $autor->cedula }}</p>
                @endif
                @endif
